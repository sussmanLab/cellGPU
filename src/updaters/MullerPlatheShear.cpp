...
*/
MullerPlatheShear::MullerPlatheShear(int period, int slabs,Dscalar boxHeight)
    {
    setPeriod(period);
    deltaP = 0;
...
*/
void MullerPlatheShear::updateSlabInfo(int slabs, Dscalar boxHeight)
    {
    Nslabs = floor(slabs/2)*2;
    boxY = boxHeight;
    slabThickness = (Dscalar)boxY/Nslabs;
...
void MullerPlatheShear::performUpdate()
    ...
    if(p1idx >=0 && p2idx >=0)
        {
        Dscalar v1 = h_v.data[p1idx].x;
        Dscalar m1 = h_m.data[p1idx];
        Dscalar v2 = h_v.data[p2idx].x;
        Dscalar m2 = h_m.data[p2idx];
        h_v.data[p2idx].x = v1;
        h_v.data[p1idx].x = v2;
        deltaP = m1*v1-m2*v2;
